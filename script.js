...
function displayResults(results) {
    ...
    if (results.length === 0) {
        resultsContainer.innerHTML = "<p>No cafes found matching the search term.</p>";
    } else {
        results.forEach(result => {
            ...
            cafeDiv.innerHTML = `
                <h3>${result.name}</h3>
                <p><strong>Address:</strong> ${result.street_no}, ${result.locality}, ${result.postal_code}</p>
                <p><strong>Latitude:</strong> ${result.lat}</p>
                <p><strong>Longitude:</strong> ${result.long}</p>
            `;
            resultsContainer.appendChild(cafeDiv);
        });
    }
}